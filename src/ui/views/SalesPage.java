...
public class SalesPage extends VBox {
    ...
    private final TextField availableStockField = Theme.textField("Stock");
    private final TextField quantityField = Theme.textField("Quantity");
    private final TextField priceField = Theme.textField("Selling price");
    private final TextField invoiceSearchField = Theme.textField("Search sales invoices");
    ...
    private final TextField paymentAmountField = Theme.textField("Amount");
    ...
    private TabPane createContent() {
        ...
        tabs.getTabs().add(new Tab("Invoices", Theme.ledgerWorkspace(createHistoryPane(), Theme.ledgerInspector("Sales Inspector", scrollEditor(editor)))));
    ...
    private GridPane createItemForm() {
        ...
        Theme.addInspectorActions(form, 5, itemActionButton, itemRemoveButton, clear);
        return form;
    }
    ...
    private BorderPane createHistoryPane() {
        ...
        pane.setCenter(Theme.ledgerSurface("Sales Invoice Ledger", toolbar, split));
        pane.setMinWidth(0);
        return pane;
    }
    ...
    private BorderPane createPaymentsPane() {
        ...
        paymentActionButton = Theme.primaryButton("Add");
        paymentDeleteButton = Theme.dangerButton("Delete");
        Button clear = Theme.secondaryButton("Clear");
        ...
        Theme.setVisible(paymentDeleteButton, false);
        Theme.addInspectorActions(form, 3, paymentActionButton, paymentDeleteButton, clear);
        ...
        BorderPane pane = Theme.ledgerWorkspace(
                Theme.ledgerSurface("Payment History", Theme.ledgerCommandBar(new Label("Payments for selected invoice")), paymentTable),
                Theme.ledgerInspector("Payment Inspector", scrollEditor(inspector))
        );
    ...
    private void configureTables() {
        itemTable.getColumns().add(TableUtil.column("Product ID", LineItem::getProductId, 90));
        itemTable.getColumns().add(TableUtil.column("Product", LineItem::getProductName, 180));
        itemTable.getColumns().add(TableUtil.column("Price", LineItem::getSellingPrice, 90));
        itemTable.getColumns().add(TableUtil.column("Quantity", LineItem::getQuantity, 80));
        itemTable.getColumns().add(TableUtil.column("Warranty", LineItem::getWarrantyEndDate, 110));
        itemTable.getColumns().add(TableUtil.column("Line Total", LineItem::getLineTotal, 110));
        ...
        invoiceTable.getColumns().add(TableUtil.column("Invoice ID", SalesInvoice::getSalesInvoiceId, 90));
        invoiceTable.getColumns().add(TableUtil.column("Date", SalesInvoice::getInvoiceDate, 110));
        invoiceTable.getColumns().add(TableUtil.column("Client ID", SalesInvoice::getClientId, 90));
        invoiceTable.getColumns().add(TableUtil.column("Client", invoice -> clientName(invoice.getClientId()), 160));
        invoiceTable.getColumns().add(TableUtil.column("Warehouse ID", SalesInvoice::getWarehouseId, 110));
        invoiceTable.getColumns().add(TableUtil.column("Warehouse", invoice -> warehouseName(invoice.getWarehouseId()), 150));
        invoiceTable.getColumns().add(TableUtil.column("Payment Type", SalesInvoice::getPaymentType, 120));
        invoiceTable.getColumns().add(TableUtil.column("Payment", SalesInvoice::getPayment, 100));
        invoiceTable.getColumns().add(TableUtil.column("Amount", SalesInvoice::getAmount, 110));
        invoiceTable.getColumns().add(TableUtil.column("Status", this::paymentStatus, 100));
        ...
        paymentTable.getColumns().add(TableUtil.column("Payment ID", SalesPayment::getSalesPaymentId, 90));
        paymentTable.getColumns().add(TableUtil.column("Date", SalesPayment::getPaymentDate, 110));
        paymentTable.getColumns().add(TableUtil.column("Amount", SalesPayment::getAmount, 110));
        paymentTable.getColumns().add(TableUtil.column("Type", SalesPayment::getPaymentType, 130));
    ...
    private void saveCurrentItem() {
        ...
        try {
            ...
            if (quantity <= 0 || price.signum() <= 0) {
                Theme.showError("Quantity and selling price must be positive.");
                return;
            }
            ...
            if (requestedAfterChange > availableAfterReverse) {
                Theme.showError("Not enough stock. Available: " + availableAfterReverse + ", already in invoice: " + currentQuantityExcludingEdit);
                return;
            }
            ...
            Theme.showError("Quantity and price must be valid.");
        }
    }
    ...
    private void saveInvoice() {
        ...
        if (salesInvoiceDAO.createSalesInvoice(invoice)) {
            Theme.showInfo("Sales invoice saved. Inventory decreased.");
            clearInvoice();
            loadData();
        } else {
            Theme.showError("Failed to save sales invoice.");
        }
    }
    ...
    private void updateInvoice() {
        ...
        if (!Theme.confirm("Update this sales invoice? Inventory will be adjusted.")) return;

        SalesInvoice invoice = buildInvoice(editingInvoiceId);
        if (invoice == null) return;

        if (salesInvoiceDAO.updateSalesInvoice(invoice)) {
            Theme.showInfo("Sales invoice updated.");
            clearInvoice();
            loadData();
        } else {
            Theme.showError("Failed to update sales invoice.");
        }
    }
    ...
    private void deleteInvoice() {
        ...
        if (!Theme.confirm("Delete sales invoice #" + invoiceId + "? Stock will be returned.")) return;

        if (salesInvoiceDAO.deleteSalesInvoice(invoiceId)) {
            Theme.showInfo("Sales invoice deleted.");
            clearInvoice();
            loadData();
        } else {
            Theme.showError("Failed to delete sales invoice.");
        }
    }
    ...
    private SalesInvoice buildInvoice(int invoiceId) {
        ...
        if (client == null || warehouse == null || currentItems.isEmpty()) {
            Theme.showError("Select client, warehouse, and add at least one item.");
            return null;
        }
    ...
    private void addPayment() {
        ...
        if (salesPaymentDAO.addPayment(payment)) {
            Theme.showInfo("Payment added.");
            refreshAfterPaymentChange();
        } else {
            Theme.showError("Could not add payment.");
        }
    }
    ...
    private void updatePayment() {
        ...
        if (selected == null) {
            Theme.showError("Select a payment first.");
            return;
        }
        ...
        if (salesPaymentDAO.updatePayment(payment)) {
            Theme.showInfo("Payment updated.");
            refreshAfterPaymentChange();
        } else {
            Theme.showError("Could not update payment.");
        }
    }
    ...
    private SalesPayment readPayment(boolean requireSelectedPayment) {
        ...
        try {
            ...
            Theme.showError("Payment amount must be valid.");
            return null;
        }

        if (amount.signum() <= 0) {
            Theme.showError("Payment amount must be positive.");
            return null;
        }

        if (paymentDatePicker.getValue() == null || paymentHistoryTypeComboBox.getValue() == null) {
            Theme.showError("Payment date and type are required.");
            return null;
        }
    ...
    private void updatePaymentFormMode() {
        ...
        if (paymentDeleteButton != null) {
            Theme.setVisible(paymentDeleteButton, selected);
        }
    }
