...
public class ReportsPage extends VBox {
    ...
    private final TextField salesSearchField = Theme.textField("Search sales report");
    private final TextField purchaseSearchField = Theme.textField("Search purchase report");
    private final TextField analysisSearchField = Theme.textField("Search analysis report");
    private final ComboBox<String> salesSearchColumnBox = TableUtil.searchColumnBox();
    private final ComboBox<String> purchaseSearchColumnBox = TableUtil.searchColumnBox();
    private final ComboBox<String> analysisSearchColumnBox = TableUtil.searchColumnBox();
    ...
    private ReportTable currentSalesModel;
    ...
    private ReportTable currentPurchaseModel;
    ...
    private ReportTable currentAnalysisModel;
    ...
    private BorderPane createSalesTab() {
        ...
        salesYearField = Theme.textField("2026");
        salesYearField.setText("2026");
        salesSummaryLabel.getStyleClass().add("muted-label");

        Button run = Theme.primaryButton("Run Report");
        Button clear = Theme.secondaryButton("Clear");
        run.setOnAction(e -> runSalesReport());
        clear.setOnAction(e -> clearSalesReport());

        HBox filters = Theme.ledgerCommandBar(salesReportComboBox, salesStartDatePicker, salesEndDatePicker, salesYearField, run, clear);
        ...
        salesSearchColumnBox.valueProperty().addListener((obs, oldText, newText) -> applySalesSearch());

        salesChartPane = new BorderPane();
        salesChartPane.setCenter(ChartUtil.barChart("Sales Chart", Map.of("No data", 0)));

        salesChartCard = Theme.ledgerSurface("Sales Chart", Theme.ledgerCommandBar(new Label("Muted report chart")), salesChartPane);
        salesChartCard.getStyleClass().add("report-chart-card");
        VBox salesResultsCard = Theme.ledgerSurface("Sales Results", createSalesSearchToolbar(), new VBox(6,
                salesSummaryLabel,
                salesTable
        ));
    ...
    private BorderPane createPurchaseTab() {
        ...
        HBox filters = Theme.ledgerCommandBar(purchaseReportComboBox, supplierComboBox, productComboBox,
        ...
        Theme.styleTable(purchaseTable);
        purchaseSearchField.textProperty().addListener((obs, oldText, newText) -> applyPurchaseSearch());
        purchaseSearchColumnBox.valueProperty().addListener((obs, oldText, newText) -> applyPurchaseSearch());

        purchaseChartPane = new BorderPane();
        purchaseChartPane.setCenter(ChartUtil.barChart("Purchase Chart", Map.of("No data", 0)));

        purchaseChartCard = Theme.ledgerSurface("Report Chart", Theme.ledgerCommandBar(new Label("Muted report chart")), purchaseChartPane);
        purchaseChartCard.getStyleClass().add("report-chart-card");
        VBox purchaseResultsCard = Theme.ledgerSurface("Purchase Results", createPurchaseSearchToolbar(), new VBox(6,
    ...
    private BorderPane createAnalysisTab() {
        ...
        HBox filters = Theme.ledgerCommandBar(analysisReportComboBox, run, clear);
        filters.getStyleClass().add("report-filter-bar");
        filters.getStyleClass().add("report-ledger-filters");
        analysisReportComboBox.setOnAction(e -> updateAnalysisChartVisibility());

        Theme.styleTable(analysisTable);
        analysisSearchField.textProperty().addListener((obs, oldText, newText) -> applyAnalysisSearch());
        analysisSearchColumnBox.valueProperty().addListener((obs, oldText, newText) -> applyAnalysisSearch());

        analysisChartPane = new BorderPane();
        analysisChartPane.setCenter(ChartUtil.barChart("Analysis Chart", Map.of("No data", 0)));

        analysisChartCard = Theme.ledgerSurface("Analysis Chart", Theme.ledgerCommandBar(new Label("Muted report chart")), analysisChartPane);
        analysisChartCard.getStyleClass().add("report-chart-card");
        VBox analysisResultsCard = Theme.ledgerSurface("Analysis Results", createAnalysisSearchToolbar(), new VBox(6,
    ...
    private HBox createSalesSearchToolbar() {
        ...
        HBox toolbar = Theme.ledgerCommandBar(salesSearchColumnBox, salesSearchField, refresh);
    ...
    private HBox createPurchaseSearchToolbar() {
        ...
        HBox toolbar = Theme.ledgerCommandBar(purchaseSearchColumnBox, purchaseSearchField, refresh);
    ...
    private HBox createAnalysisSearchToolbar() {
        ...
        HBox toolbar = Theme.ledgerCommandBar(analysisSearchColumnBox, analysisSearchField, refresh);
    ...
    private void runSalesReport() {
        try {
            ...
            TableUtil.populateSearchColumns(salesSearchColumnBox, model);
            applySalesSearch();
        } catch (Exception e) {
            Theme.showError("Could not run sales report: " + e.getMessage());
        }
    }
    ...
    private void runPurchaseReport() {
        try {
            ...
            TableUtil.populateSearchColumns(purchaseSearchColumnBox, model);
            ...
            Theme.showError("Could not run purchase report: " + e.getMessage());
        }
    }
    ...
    private void runAnalysisReport() {
        try {
            ...
            TableUtil.populateSearchColumns(analysisSearchColumnBox, model);
            analysisSummaryLabel.setText(report + " | Rows: " + model.getRowCount());
            setAnalysisChart(report, purchaseChartData(report, model));
            applyAnalysisSearch();
        } catch (Exception e) {
            Theme.showError("Could not run analysis report: " + e.getMessage());
        }
    }
    ...
    private ReportTable modelFromRows(String[] columns, List<Object[]> rows) {
    ...
    private void applySalesSearch() {
        filterModel(currentSalesModel, salesTable, salesSearchField.getText(), salesSearchColumnBox.getValue());
    }

    private void applyPurchaseSearch() {
        filterModel(currentPurchaseModel, purchaseTable, purchaseSearchField.getText(), purchaseSearchColumnBox.getValue());
    }

    private void applyAnalysisSearch() {
        filterModel(currentAnalysisModel, analysisTable, analysisSearchField.getText(), analysisSearchColumnBox.getValue());
    }

    private void filterModel(ReportTable source, TableView<Map<String, Object>> table, String keyword, String selectedColumn) {
        if (source == null) return;

        ReportTable filtered = new ReportTable();
        ...
        for (int row = 0; row < source.getRowCount(); row++) {
            if (text.isEmpty() || rowMatches(source, row, text, selectedColumn)) {
        ...
        TableUtil.fillFromModel(table, filtered);
    }

    private boolean rowMatches(ReportTable model, int row, String keyword, String selectedColumn) {
        for (int col = 0; col < model.getColumnCount(); col++) {
            if (selectedColumn != null
                    && !TableUtil.ALL_SEARCH_COLUMNS.equals(selectedColumn)
                    && !model.getColumnName(col).equals(selectedColumn)) {
                continue;
            }
    ...
    private Map<String, Number> purchaseChartData(String report, ReportTable model) {
    ...
    private Map<String, Number> chartFromModel(ReportTable model, String labelColumn, String valueColumn, int limit) {
    ...
    private Map<String, Number> latestValueChart(ReportTable model, String labelColumn, String valueColumn) {
        Map<String, Number> values = new LinkedHashMap<>();
        int labelIndex = columnIndex(model, labelColumn);
        int valueIndex = columnIndex(model, valueColumn);
        if (labelIndex < 0 || valueIndex < 0) return Map.of("No chart", 0);
    ...
    private BigDecimal sum(ReportTable model, String columnName) {
    ...
    private int columnIndex(ReportTable model, String columnName) {
    ...
    private void setSalesChart(String title, Map<String, Number> data) {
        salesChartPane.setCenter(ChartUtil.barChart(title, data));
    }

    private void setSalesChart(String title, Map<String, Number> data, boolean connectedPlot) {
        salesChartPane.setCenter(connectedPlot ? ChartUtil.connectedPlot(title, data) : ChartUtil.barChart(title, data));
    }

    private void setPurchaseChart(String title, Map<String, Number> data) {
        purchaseChartPane.setCenter(ChartUtil.barChart(title, data));
    }

    private void setPurchaseChart(String title, Map<String, Number> data, boolean connectedPlot) {
        purchaseChartPane.setCenter(connectedPlot ? ChartUtil.connectedPlot(title, data) : ChartUtil.barChart(title, data));
    }

    private void setAnalysisChart(String title, Map<String, Number> data) {
        analysisChartPane.setCenter(ChartUtil.barChart(title, data));
    }
    ...
    private void clearSalesReport() {
        ...
        salesSearchColumnBox.getItems().setAll(TableUtil.ALL_SEARCH_COLUMNS);
        salesSearchColumnBox.getSelectionModel().select(TableUtil.ALL_SEARCH_COLUMNS);
    ...
    private void clearPurchaseReport() {
        ...
        purchaseSearchColumnBox.getItems().setAll(TableUtil.ALL_SEARCH_COLUMNS);
        purchaseSearchColumnBox.getSelectionModel().select(TableUtil.ALL_SEARCH_COLUMNS);
    ...
    private void clearAnalysisReport() {
        ...
        analysisSearchColumnBox.getItems().setAll(TableUtil.ALL_SEARCH_COLUMNS);
        analysisSearchColumnBox.getSelectionModel().select(TableUtil.ALL_SEARCH_COLUMNS);
