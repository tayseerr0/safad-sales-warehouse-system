...
public class DashboardPage extends VBox {
    ...
    public DashboardPage(Consumer<String> navigator) {
    ...
    private Node createChartSlideshow() {
        ...
        Button previous = Theme.secondaryButton("Previous");
        Button next = Theme.secondaryButton("Next");
        ...
        HBox controls = Theme.ledgerCommandBar(previous, next);
    ...
    private Node createOperationsAlertsPane() {
        ...
        return Theme.ledgerSurface("Operations + Alerts", Theme.ledgerCommandBar(new Label("Daily work")), content);
    }
    ...
    private Node createOperationsPanel() {
        ...
        return Theme.ledgerSurface("Operations", Theme.ledgerCommandBar(new Label("Alerts and capacity")), content);
    }
    ...
    private Node createWorkbenchPanel() {
        ...
        VBox.setVgrow(content.getChildren().get(2), Priority.ALWAYS);
        return Theme.ledgerSurface("Workbench", Theme.ledgerCommandBar(new Label("Open and inspect")), content);
    }
    ...
    private Node createChartSlide() {
        ...
        if (chartSlideIndex == 0) {
            chart = ChartUtil.connectedPlot(
                    "Sales by Month - " + LocalDate.now().getYear(),
                    monthlySales()
            );
        } else {
            chart = ChartUtil.barChart(
    ...
    private VBox createWarehouseLoadPane() {
        return Theme.ledgerSurface("Warehouse Board", Theme.ledgerCommandBar(new Label("Capacity use")), createWarehouseLoadList());
    }
    ...
    private Node createShortcutsSnapshotPane() {
        ...
        content.getChildren().forEach(node -> HBox.setHgrow(node, Priority.ALWAYS));
        return Theme.ledgerSurface("Workbench + Snapshot", Theme.ledgerCommandBar(new Label("Open and inspect")), content);
    }
    ...
    private Button actionButton(String text, String page, boolean primary) {
        Button button = primary ? Theme.primaryButton(text) : Theme.secondaryButton(text);
